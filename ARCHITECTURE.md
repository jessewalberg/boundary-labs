...
The architecture is deliberately multi-agent — a single-agent or linear pipeline is rejected by the PRD. Four agent roles are mandatory: the **Orchestrator** decides what to test next, the **Red Team Agent** generates and mutates attacks, the **Judge** evaluates whether attacks succeeded (in a context independent of the attack generator), and the **Documentation Agent** converts confirmed findings into professional vulnerability reports. Three deterministic supporting services handle policy- and schema-driven work without LLM discretion: a **Safety Gate Service** for authorization, allowlists, PHI handling, cost limits, and approval gating; a **Coverage Scoring Service** that maintains the threat-model coverage map; and a **Regression Promotion Service** that converts confirmed exploits into versioned evals. Each agent has its own context, tools, trust level, inputs, outputs, and stop conditions.

The platform is a **separate app from the Clinical Co-Pilot target**. The target repo already has a Python 3.12 FastAPI `clinical-copilot` service, Pydantic AI agents, Pydantic Evals, Logfire/OpenTelemetry tracing, SMART-on-FHIR sessions, OpenEMR module integration, golden evals, planted-regression checks, rate limits, and Railway deployment config. Boundary runs in its own workspace and reaches the deployed target only through authorized HTTP/SSE interfaces. The Target Adapter is the only component that talks to the Co-Pilot; agents never receive broad network access or production credentials; all test data is synthetic or explicitly approved; target output is always untrusted. The whole stack is Pydantic end to end — Pydantic models for contracts, Pydantic AI for agents, `pydantic_graph.Graph` for orchestration, Pydantic Evals for regression suites, Logfire/OpenTelemetry for traces.

Priority attack categories follow the PRD and current LLM security guidance: direct/indirect/multi-turn prompt injection, data exfiltration, cross-patient authorization bypass, state corruption, tool misuse, denial of service and cost amplification, and identity/role exploitation. Per-category mappings to OWASP LLM Top 10 and MITRE ATLAS live in `THREAT_MODEL.md`; the platform-level NIST AI RMF alignment (GOVERN/MAP/MEASURE/MANAGE) lives in the Framework Alignment section below.

The trust model is conservative. The system may autonomously generate low-risk variants, run authorized campaigns, record results, draft reports, and schedule regressions. It must stop for human approval before testing any non-allowlisted target, using real PHI, filing high/critical reports externally, recommending production remediation, changing target defenses, or expanding privileges. Autonomy increases coverage and repeatability; high-impact actions remain governed, auditable, and reversible.

Implementation shape: a single Docker container runs the Next.js console and the Python Pydantic Graph worker under `supervisord`. Better Auth protects the console with three Boundary roles (`admin`, `operator`, `reviewer`). SQLite on `/data/boundary.db` is the system of record (WAL mode, append-only `audit_events`, `campaign_jobs` queue, artifact storage under `/data/artifacts`). Safety Gate policy lives in `policy_values`, mirrored into the worker by codegen and enforced at every mutation. The CISO-readable trust surface is `/settings/policy`, `/settings/baa`, `/audit`, `/approvals`, and `/reports`.
...
Regression harness update: confirmed exploits now have dedicated regression case, case-version, suite, suite-result, target-version, lifecycle, cost, and agent-timeline tables. `regression_suite` worker jobs load active promoted cases from SQLite instead of the exploratory seed corpus, carry target-version metadata into artifacts, and materialize pass/fail/partial/invalid outcomes through ingest. Missing required evidence is persisted as `invalid`, not `pass`; later failures after a prior pass create `reopened` lifecycle events and may be marked as cross-category regressions. Operators use `/regressions` and the read-model repositories for resilience, cost, and ordered agent activity.
...
Boundary is intentionally grounded in three industry frameworks. The mapping is at the **platform** level here; the **per-category** mapping (which OWASP LLM item and MITRE ATLAS technique each attack class belongs to) lives in `THREAT_MODEL.md` under "Mapped Frameworks."

### NIST AI Risk Management Framework (AI RMF 1.0)

| NIST AI RMF function | Boundary surface | Where in this platform |
| --- | --- | --- |
| **GOVERN** — policies, accountability, lifecycle controls | Policy table, BAA acknowledgement, approval workflow, audit ledger | `policy_seed.json`, `apps/web/src/server/safety-gate/*`, `/settings/policy`, `/settings/baa`, `/approvals`, immutable `audit_events` table |
| **MAP** — context, capabilities, risks identified | Threat model, attack surface map, coverage prioritization | `THREAT_MODEL.md`, `/threat-model` route, Coverage Scoring Service (`apps/web/src/server/coverage/query.ts`) |
| **MEASURE** — quantitative and qualitative measurement of identified risks | Eval suite, judge verdicts, regression harness, cost telemetry, observability layer | `evals/`, `verdicts` table, `regression_cases` + `regression_suite_results`, `run_costs`, `agent_timeline_events`, `/dashboard` + `/coverage` + `/regressions` |
| **MANAGE** — prioritize and act on risks | Orchestrator scheduling, regression promotion, lifecycle events, vulnerability reports | Orchestrator Agent (`worker/graphs/campaign.py`, `orchestrator-state` repository), Regression Promotion Service, `vulnerability_lifecycle_events`, `/reports` |

The four functions are not implemented as separate modules — they are properties of the existing surfaces. The mapping is auditable in the sense that a reviewer can point at any RMF function and find the concrete file, route, and table that addresses it.

### OWASP Top 10 for LLM Applications (2025)

Per-category mapping lives in `THREAT_MODEL.md`. At the platform level, three Top 10 items shape design:

- **LLM01 (Prompt Injection)** — the dominant attack class probed by the Red Team Agent; the Judge Agent's deterministic + LLM-judge layering exists specifically to prevent the attack generator from confirming its own success.
- **LLM06 (Excessive Agency)** — every Pydantic AI tool the target exposes is treated as part of the attack surface. The Documentation Agent's reports include tool-misuse subcategories explicitly.
- **LLM10 (Unbounded Consumption)** — the Cost And Scale Strategy section (above) and the per-attempt frontier cap + low-signal stop rule are direct responses to LLM10.

### MITRE ATLAS

Per-category technique mapping lives in `THREAT_MODEL.md`. At the platform level, ATLAS provides the vocabulary for documenting *how* the Red Team mutates attacks (variants → escalations → multi-turn sequences) and *why* an attack class is worth regression coverage even when the current target version refuses the seed prompt. ATLAS technique IDs appear in vulnerability reports alongside the OWASP LLM mapping for cross-domain traceability.

### Living-document commitment

The frameworks are anchors, not freezes. Boundary's `MEASURE` and `MANAGE` surfaces are designed to update the `MAP` artifact on every run: judge verdicts, regression promotions, and discovered new attack classes feed back into the threat model and the framework mappings.
...
These are open items that ship as future work in the submission state. Each is a deliberate scope cut, documented here so a reviewer can verify the gap is acknowledged rather than overlooked. Where work is already partially complete (schema and UI in place, code path missing), the location of the missing piece is cited so a future contributor can resolve it in one or two patches.

| # | Limitation | Where it surfaces today | Where it gets fixed | Risk if shipped as-is |
|---|---|---|---|---|
| 1 | **Worker `run_costs` ingest not wired.** The schema, repository, and `CostBreakdown` UI component exist; `worker/artifact_ingest.py` does not yet emit cost rows. | `apps/web/var/boundary.db` `run_costs` table is empty | Add `INSERT INTO run_costs` block in `apps/web/src/server/ingest/from-artifact.ts` reading `artifact.documentation_agent`-adjacent cost block | `AI_COST_ANALYSIS.md` actuals remain estimated rather than provider-reported. |
| 2 | **Documentation Agent live worker → ingest → `reports` loop.** The 3 vuln reports were derived via `apps/web/scripts/derive-reports.ts`; the live worker `DocumentationNode` emits a thin dict that ingest does not persist. | `worker/graphs/campaign.py:483-528` emit + `apps/web/src/server/ingest/from-artifact.ts` (no `INSERT INTO reports`) | Extend DocumentationNode prompt to emit the full template fields, add JSON validation, and persist via `insertReport()` in ingest | Manual derivation step required after every campaign; the regenerator script is idempotent so submission impact is bounded. |
| 3 | **Mid-run cost / low-signal halt rule.** Defined in policy (`ARCHITECTURE.md:809-813`) but not enforced at runtime. | `worker/graphs/campaign.py` has no per-case budget guard | Add per-case spend rollup in `CampaignGraphState`, early-`End` branch when `cumulative_cost > budget_cents`, emit `halt_reason` | Worst-case runaway dev spend on adaptive mutation loops; bounded today only by `BOUNDARY_ADAPTIVE_ATTACK_LIMIT` (default 4). |
| 4 | **Orchestrator agent depth.** The Python orchestrator path (`worker/graphs/orchestrator.py`) is a connection ping; live category scheduling falls back to `coverage_gap_schedule()` which sorts categories alphabetically. The real prioritizer at `apps/web/src/server/orchestrator-state/repository.ts` is consumed by `/regressions` UI but not by the worker. | Agent timeline shows orchestrator as a no-op step | Have worker poll `orchestrator-state` over HTTP or a shared DB read; pass `recommended_categories` into `CoverageScoreNode` | Coverage is driven by seed file order, not by gap density. Demo-visible. |
| 5 | **Per-role provider routing not exercised.** `worker/llm_provider.py:73,83-84` supports `BOUNDARY_<ROLE>_MODEL` and `BOUNDARY_<ROLE>_PROVIDER` overrides; in practice every role uses `google/gemini-2.5-flash` via OpenRouter. | `var/provider-proof-output.json` `agent_connections` shows identical model for all four roles | Set distinct defaults per role (e.g. `judge=anthropic/claude-sonnet-4-6`, `red_team=openai/gpt-4o-mini`) and document in deployment config | Judge independence from Red Team is structural (different node, different prompt) but not provider-isolated. |
| 6 | **Mutation loop is one-shot.** `AdaptiveRedTeamNode` (`worker/graphs/campaign.py:282-338`) runs once after the initial pass; hard-capped at 1-12 variants total per run; does not iterate per partial verdict. Multi-turn capped at 3 turns (`campaign.py:884,912`). | Adaptive output produces 1-12 follow-ups, not "ten variants per partial" | Wrap the adaptive call in a verdict-driven loop with a per-partial budget; bump `BOUNDARY_MULTI_TURN_LIMIT` for high-priority cases | Mutation depth is shallower than the assignment's "ten variants" framing implies. |
| 7 | **No production caller for `enqueueRegressionSuite`.** The function exists; only a test imports it. The `/schedule` page advertises an `orchestrator_sweep_cadence` policy that no scheduler enacts. | `/schedule` policy rendered, no run starts on cadence | Add a "Run regression suite now" admin button on `/schedule` that calls a server action → `enqueueRegressionSuite()`. Defer cron. | Regression coverage exists in the schema and UI but is only triggered by manual UI action; visible inconsistency. |
| 8 | **Cross-category regression flag is dead-coded.** `enqueueRegressionSuite` (`apps/web/src/server/regression-suites/repository.ts:69-175`) does not populate `fixed_category`; `worker/artifact_ingest.py:196-199` reads it and so `is_cross_category_regression` is always false. | `/regressions` "cross-category" chip never lights | Look up the case's `fixed_in_category` from `regression_cases` during enqueue and include in the suite payload | Cannot detect "fixing X broke Y" today; primary regression detection ("X reappeared") still works. |
| 9 | **Evidence matching is path-only string match.** `apps/web/src/server/regression-suites/classify-result.ts:13-33` only matches `evidence.type === "path"` against `turn.endpoint`. `deterministicChecks` and `invalidConditions` schemas are persisted at promotion but never executed by the classifier. | Promoted-case detail page shows the rubric; classifier ignores it | Wire `deterministicChecks` and `invalidConditions` into `classifyRegressionResult` with simple shape support (status code, header present, body contains) | Risk that a regression test passes because model behavior shifted, not because the vulnerability was fixed — the exact failure mode the assignment calls out. |
| 10 | **`p1_live_chat_break_cases.json` not in latest live run.** 8 seeded P1 cases (`data_exfiltration`, `availability_abuse`) are loaded but the most recent live MVP run only executed the 4 P0 cases. | `evals/results/latest.json` totals `4 / 4 pass`; P1 corpus has no recent observed verdict | Re-run `pnpm derive:reports` or `scripts/run_proof_campaign.py` with both seed files merged | Coverage of `data_exfiltration` and `availability_abuse` categories shown only in the seed corpus, not in live verdicts. |

Items 3, 7, 8, and 9 are the highest-leverage if a future submission cycle has time — items 3 and 7 are the strongest demo additions (visible cost-halt event; visible regression sweep trigger), items 8 and 9 close gaps the assignment explicitly calls out ("worse than no test").
